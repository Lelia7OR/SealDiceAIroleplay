...
SealDiceAIres 是一个海豹骰 JS 插件，通过接入 AI API，让你的骰子可以扮演自定义的角色人设。通过指令，玩家可以直接和骰子对话，骰子会以该角色的身份回应。
...
下载本仓库中的 `sealdiceAIres.js`，用文本编辑器打开，找到顶部配置区填写：
...
进入海豹骰后台 → 扩展功能 → JS扩展 → 上传插件，选择 `sealdiceAIres.js`。
...
> 💡 等待提示语来自 JS 插件中的设置，你可以在 `sealdiceAIres.js` 里找到并修改成你喜欢的文字。
...
SealDiceAIres 的初衷绝对不是认为AI创作可以替代人类创作！我只是试图在原本的固定自定义回复中，多探索一些机动的可能性，从而丰富体验。
